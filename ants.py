...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        ant_presence = self.place.ant
        blocked = ant_presence is not None and ant_presence.blocks_path == True
        return blocked
        # END Problem 7
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        ant_place, index = self.place, 0
        while ant_place != beehive:
            if ant_place.bees and (self.min_range <= index <= self.max_range):
                return random_or_none(ant_place.bees)
            ant_place, index = ant_place.entrance, index + 1
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting != 0:
            ...
            ant_place = self.place
            random_bee = random_or_none(ant_place.bees)
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.imposter == True:
            ...
            ant_place = self.place
            while ant_place.exit:
                that_ant = ant_place.exit.ant
                if that_ant:
                    if that_ant not in self.doubled_ants:
                        that_ant.damage *= 2
                        self.doubled_ants.append(that_ant)
                if isinstance(that_ant, BodyguardAnt):
                    if that_ant.contained_ant:
                        if that_ant.contained_ant not in self.doubled_ants:
                            that_ant.contained_ant.damage *= 2
                            self.doubled_ants.append(that_ant.contained_ant)
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    previous_action, new_action = bee.action, effect(bee.action, bee)
